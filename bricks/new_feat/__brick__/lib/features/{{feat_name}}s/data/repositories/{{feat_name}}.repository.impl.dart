...
import 'package:popy_shop/core/helpers/handle_exception.dart';

import 'package:popy_shop/features/{{feat_name}}s/data/datasources/{{feat_name}}.datasource.dart';
...
class {{feat_name.pascalCase()}}RepositoryImpl extends {{feat_name.pascalCase()}}Repository {
  ...
  @override
  Future<Either<Failure, void>> create({{feat_name.pascalCase()}}RequestModel params) async {
    return await handleException(() async {
      return await {{feat_name}}Datasource.create(params);
    });
  }

  @override
  Future<Either<Failure, void>> update({{feat_name.pascalCase()}}RequestModel params) async {
    return await handleException(() async {
      return await {{feat_name}}Datasource.update(params);
    });
  }
